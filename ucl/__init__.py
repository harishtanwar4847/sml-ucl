...
def lead_dashboard_list():
    user = __user()
    lead = frappe.get_all("Lead", filters= {"owner": user.name}, fields = ["name", "mobile_number", "email_id", "full_name", "pan_number", "workflow_state", "sub_product", "requested_loan_amount", "rc_number", "monthly_income", "obligations"])
    if len(lead) == 0:
        dict = {"lead_list": lead}
    else:
        dict = {"lead_list": lead}
