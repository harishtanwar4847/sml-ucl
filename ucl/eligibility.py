...
def ibb(**kwargs):
    try:
        ...
        if response.status_code == 200:
            ...
            ucl.log_api_response(is_error = 1, error  = frappe.get_traceback(), api_log_doc = api_log_doc, api_type = "Third Party", response = str(response.json(), status_code=response.status_code), status_code=response.status_code)
